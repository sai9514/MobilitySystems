...
for week in user_weekly_trips:
    ...
    for trip in trips:
        ...
        x[tripNum] = m.addVar(vtype=GRB.BINARY, name='publicUsage_trip_' + str(
            tripNum))  # Decision variable to check if public transport was used in this trip
        ...
        print("Total No. of edges after orig, dest and e scooters: ", nx.number_of_edges(G))
                ...
                if 'walk' != modeKey:
                    if 'scoot' != modeKey:
                        publicEdgeAttrs[tripNum, (eachEdge[0], eachEdge[1], modeKey)] = eachEdge[
                            2]  # Public transport edges for each trip
                    else:
                        scootEdgeAttrs[tripNum, (eachEdge[0], eachEdge[1], modeKey)] = eachEdge[
                            2]  # EScooter edges for each trip
        ...
        for edges in edgeAttrs:
            ...
            if edges[0] == tripNum:
                ...
                v = edges[1][1]  # to node
                r[edges] = m.addVar(vtype=GRB.BINARY, name='r_' + str(tripNum) + str(
                    edges))  # route decision variable based on tripNum, and edges
            ...
            subNodesList.append(list(G.nodes()))  # list of nodes in each trip
    ...
    for nodes in subNodesList:  # removing orig and dest in each list of nodes
        nodes.remove('orig')
        nodes.remove('dest')
        # print(*nodes)
    ...
    with open(directory + 'ObjectiveValuesWeekly.csv', 'a') as csv_file:
        ...
        writer.writerow([week, "Monetary Time Value", obj.getValue() - WeeklyPackageCost - (EScooterCost *
                        scooterOverUsage) - (publicTransportCost * publicTransOverUsage)])
